...
from loss_function_gpu import CFLossFunc
from network_module_gpu import SampleNet, AdvNet, InnerProductDecoder
from utility_gpu import prepare_dir, get_input_white_noise, get_graph_target, record_as_img, avg_record, \
...
class Model:
    ...
    def train(self, load):
        ...
        # torch.cuda_is_available() function is not needed because the availability of CUDA on Macbook M1's GPU is not supported by PyTorch currently. The code above checks if CUDA is available and assigns the device as "cuda" if it is, otherwise "cpu".
        # switching to the use of Mac M1 architecture compatibl mps
...
#                training_loss.append(observation_ae_loss)
            ...
            hidden_emb = hidden_emb.to(torch.device('mps'))
...
#        print('Validation now ------------')
        ...
        np.savetxt('accuracies_gpu_citeseer.txt', np.column_stack((training_roc, training_ap, test_roc, test_ap)))




        # Print Graph for epoch (arbitrarily set for loop), and training/ testing score (ROC, AP)
#        epochs = range(1, len(training_roc) + 1)
#        plt.plot(epochs, training_roc, '--',  label="Training ROC")
#        plt.plot(epochs, test_roc, label="Test ROC")
#
#        plt.xlabel("Epoch", fontweight='bold'), plt.ylabel("Accuracy Score", fontweight='bold'), plt.legend(loc="best")
