...
#    if source == 'cora'or 'citeseer'or 'pubmed'or 'nell.0.1':
    names = ['x', 'tx', 'allx', 'graph']
    objects = []
    for i in range(len(names)):
        with open("/Users/adriankwan_mba/Downloads/grcf-master/data/ind.{}.{}".format(source, names[i]), 'rb') as rf:
            u = pkl._Unpickler(rf)
            u.encoding = 'latin1'
            cur_data = u.load() #load pickled into volatile memory
            objects.append(cur_data) #add to objects array
        # objects.append(
        #     pkl.load(open("data/ind.{}.{}".format(dataset, names[i]), 'rb')))
    x, tx, allx, graph = tuple(objects) #make tuple with objects
    test_idx_reorder = parse_index_file(
        "/Users/adriankwan_mba/Downloads/grcf-master/data/ind.{}.test.index".format(source)) #strip of string in front and at the back, leaving only the index
    ...
    features = sp.vstack((allx, tx)).tolil() #scipy sparse, v stack- stack sparse matrices vertically (row wise), allx superset of x, feature vector of all training instances- both labelled and unlabelled; tx feature vector of test instances; made to list of lists
    features[test_idx_reorder, :] = features[test_idx_range, :]
    features = torch.FloatTensor(np.array(features.todense())) #returns matrix form of it;
    adj = nx.adjacency_matrix(nx.from_dict_of_lists(graph))

    return adj, features

#get_graph_target called in cfgan_model, need to check paper for file format, and verify the output of the function
...
#def get_graph_target(source):
#    print('Loading {} dataset...'.format(source))
#
#    idx_features_labels = np.genfromtxt("/Users/adriankwan_mba/Downloads/grcf-master/data/{}.content".format(source),
#                                        dtype=np.dtype(str))
...
#    edges_unordered = np.genfromtxt("/Users/adriankwan_mba/Downloads/grcf-master/data/{}.cites".format(source),
#                                    dtype=np.int32)
...
#    # setting 1
#    num_test = int(np.floor(edges.shape[0] / 5.))  # Double the size of the test set, 20% as test edges
#    num_val = int(np.floor(edges.shape[0] / 20.)) #5% as validation edges
#
...
#    train_edges = np.delete(edges, np.hstack([test_edge_idx, val_edge_idx]), axis=0) #75% as training edges

    # setting 2
    num_test = int(np.floor(edges.shape[0] / 10.)) #10% kept as test edges
    num_val = int(np.floor(edges.shape[0] / 20.)) #5% kept as validation edges

    all_edge_idx = list(range(edges.shape[0]))
    np.random.shuffle(all_edge_idx)
    val_edge_idx = all_edge_idx[:num_val]
    test_edge_idx = np.random.choice(all_edge_idx[num_val:(num_val + num_test)], size=int(num_test * 0.5), replace=False) #8% test edges
    test_edges = edges[test_edge_idx]
    val_edges = edges[val_edge_idx]
    train_edges = np.delete(edges, np.hstack([test_edge_idx, val_edge_idx]), axis=0) #85% as training edges

    frac = 0.75  #Preserve a fraction of train_edges to preserve
    num_train = int(frac * len(train_edges))
    
    random.seed()
    preserved_indices = random.sample(range(len(train_edges)), num_train)
    train_edges = train_edges[preserved_indices]
    # train_edges = train_edges[:num_train]
